...
public class TaskService : BackgroundService
{
    ...
    public TaskService(ILoggerFactory loggerFactory, IConfiguration env, IServiceScopeFactory scopeFactory)
    {
        _scopeFactory = scopeFactory;
        this._logger = loggerFactory.CreateLogger<TaskService>();
        // _context = context;
        _env = env;
        InitRabbitMQ();
    }
    ...
    private void HandleMessage(string content)
    {
        ...
        TaskProcessItem taskProcessItem = JsonSerializer.Deserialize<TaskProcessItem>(content);

        // string message = string.Empty;

        using (var scope = _scopeFactory.CreateScope())
        {
            var db = scope.ServiceProvider.GetRequiredService<TaskProcessContext>();
            db.Database.EnsureCreated();
            db.TaskProcessItems.Add(taskProcessItem);
            db.SaveChanges();
        }
